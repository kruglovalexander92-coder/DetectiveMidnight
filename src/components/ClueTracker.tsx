      ...
      <div className="border border-white/10 bg-[#0a0a0a] rounded-none p-4 shadow-2xl">
        <h3 className="font-sans text-[11px] font-bold text-white/95 uppercase tracking-[0.25em] border-b border-white/10 pb-2.5 mb-4 flex items-center justify-between">
          ...
          <span className="text-black bg-white px-2 py-0.5 text-[9px] font-mono font-bold tracking-normal">
            {foundClueIds.length + (activeTornNote?.completed ? 1 : 0)} / {3 + (activeTornNote?.completed ? 1 : 0)} НАЙДЕНО
          </span>
        </h3>
        ...
        <div className="w-full">
          <div className="grid grid-cols-3 gap-3">
            {currentClues.map((clue, idx) => {
              ...
              return (
                <div 
                  ...
                  className={`relative aspect-[3/4] border rounded-none flex flex-col justify-between p-2 transition-all duration-300 ${
                    isFound 
                      ? 'border-white/30 bg-[#0e0e0e] text-white shadow-xl transform rotate-1 cursor-pointer hover:border-white/60 hover:scale-[1.02]' 
                  ...
                  {isFound ? (
                    <>
                      <div className="flex justify-between items-start mb-1">
                        ...
                        <IconComponent name={clue.icon} className="w-3 h-3 text-white/60" />
                      </div>
                      
                      {clue.image && (
                        <div className="w-full aspect-[4/3] bg-stone-900 border border-white/10 overflow-hidden mb-1 relative group shrink-0">
                          <img 
                            src={clue.image} 
                            alt={clue.name} 
                            referrerPolicy="no-referrer" 
                            className="w-full h-full object-cover grayscale brightness-90 contrast-110 group-hover:grayscale-0 transition-all duration-300" 
                          />
                        </div>
                      )}

                      <div className="flex-1 flex flex-col justify-center min-h-[42px] overflow-hidden">
                        <h4 className="font-serif text-[11px] sm:text-[12px] font-bold leading-tight mb-0.5 line-clamp-1 text-stone-100">
                          {clue.name}
                        </h4>
                        <p className="font-serif italic text-[9px] sm:text-[10px] leading-tight text-white/50 overflow-y-auto max-h-[42px] custom-scrollbar">
                          {clue.description}
                        </p>
                      </div>
            ...
            {/* If there is an active torn note, render it as a 4th clue card! */}
            {activeTornNote && (
              <div 
                onClick={() => {
                  if (activeTornNote.completed) {
                    setActiveDetail({
                      type: 'clue',
                      name: 'Восстановленная записка',
                      description: activeTornNote.fullText,
                      icon: 'FileCheck',
                      idx: 4,
                ...
                className={`relative aspect-[3/4] border rounded-none flex flex-col justify-between p-2 transition-all duration-300 ${
                  activeTornNote.completed
                    ? 'border-amber-500/30 bg-[#16120e] text-white shadow-xl transform rotate-[-1deg] cursor-pointer hover:border-amber-500/60 hover:scale-[1.02]'
                    : 'border-amber-500/40 bg-[#130d07] text-amber-500/80 shadow-lg cursor-pointer hover:border-amber-500 hover:scale-[1.02]'
                }`}
                title={activeTornNote.completed ? "Нажмите для подробного просмотра" : "Нажмите, чтобы собрать пазл"}
              >
                <div className="flex justify-between items-start mb-1">
                  <span className={`text-[8px] font-mono font-bold px-1 ${
                    ...
                    {activeTornNote.completed ? 'УЛИКА №4' : 'РАЗОРВАНА'}
                  </span>
                  {activeTornNote.completed ? (
                    <Lucide.FileCheck className="w-3.5 h-3.5 text-amber-500" />
                  ) : (
                    <Lucide.Puzzle className="w-3.5 h-3.5 text-amber-500 animate-pulse" />
                  )}
                </div>
                
                <div className="w-full aspect-[4/3] bg-stone-900 border border-white/10 overflow-hidden mb-1 relative group shrink-0">
                  <img 
                    src="/src/img/Caseboard/Caseboard_evidence_11.png" 
                    alt="Записка" 
                    referrerPolicy="no-referrer" 
                    className={`w-full h-full object-cover grayscale brightness-90 contrast-110 group-hover:grayscale-0 transition-all duration-300 ${
                      !activeTornNote.completed ? 'blur-[1px] opacity-40' : ''
                    }`} 
                  />
                </div>

                <div className="flex-1 flex flex-col justify-center min-h-[42px] overflow-hidden">
                  <h4 className="font-serif text-[11px] sm:text-[12px] font-bold leading-tight mb-0.5 line-clamp-1 text-amber-100">
                    {activeTornNote.completed ? 'Записка из урны' : 'Обрывки записки'}
                  </h4>
                  <p className="font-serif italic text-[9px] sm:text-[10px] leading-tight text-white/50 overflow-y-auto max-h-[42px] custom-scrollbar">
                    {activeTornNote.completed ? activeTornNote.fullText : 'Кусочки порваны и лежат в беспорядке.'}
                  </p>
                </div>
      ...
      {activeDetail && (
        ...
        >
          ...
          >
            ...
            {/* Image or Icon */}
            {activeDetail.image ? (
              <div className="w-48 h-36 border border-white/10 bg-stone-900 overflow-hidden mb-5 mt-2 shadow-2xl relative">
                <img 
